...
import { applyContactServerErrors } from '@/core/contact/applyContactServerErrors';
...
export function ContactWithPlanForm() {
  ...
  const validationMessages = {
    ...
    messageOrConfigs: validation('messageOrConfigs')(
      'Please either select GPU configurations or provide details here.'
    )()
  };
  const {
    register,
  ...
  useEffect(() => {
    ...
    const itemsChanged = !!prevSignature && prevSignature !== itemsSignature;
  ...
  }, [formValues, itemsSignature, formStatus.type, clearErrors]);
  ...
  const onSubmit = async (data: ContactFormData) => {
    ...
    const formMessage = applyContactServerErrors(
      result.error,
      setError,
      clearErrors,
      fallback
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden">
        ...
        <div className="flex flex-col lg:min-h-0 lg:overflow-hidden">
          <div className="mb-4 shrink-0">
            ...
            <p className="text-fg-soft text-xs leading-relaxed">
              {contactT('subtitle')(
                "Share your GPU configuration requirements and we'll get back to you with a custom quote."
              )()}
            </p>
          </div>
            ...
            <div className="mb-3">
              ...
              <Textarea
                ...
                placeholder={t('form.placeholders.message')(
                  'Add extra details here—or skip the plan and describe everything here.'
                )()}
                className={cn(
            ...
            {hasIncomplete && (
              <p className="text-fg-muted mt-2 shrink-0 text-xs">
                {t('selected.confirmDuringCall')(
                  "We'll confirm provider/region availability during the call."
                )()}
              </p>
            )}
            <p className="text-fg-muted mt-1 shrink-0 text-xs">
              {t('selected.hint')(
      ...
      <div className="border-border/40 shrink-0 pt-3 pb-1">
        <p className="text-fg-main text-sm leading-relaxed font-medium">
          {t('help.description')(
            'Stuck? We’ll help pick hardware, providers, and hybrid setups. Share blockers or wishlist items and we’ll factor them in.'
          )()}
        </p>
